...
export const MemberProfileView: React.FC<{ memberId: string; onBack: () => void }> = ({ memberId, onBack }) => {
  ...
  const activeLoans = useMemo(() => {
    return memberLoans.filter(l => l.status === 'active' || l.status === 'pending');
  }, [memberLoans]);

  const primaryActiveLoan = activeLoans[0];

  const activeLoanRemainingInstallments = useMemo(() => {
    if (!primaryActiveLoan || !primaryActiveLoan.installments) return 0;
    return primaryActiveLoan.installments.filter(i => i.status === 'unpaid' || i.status === 'overdue').length;
  }, [primaryActiveLoan]);

  // Last payment transaction (Deposit, Loan Installment, Share, DPS, FDR, etc.)
  const lastPaymentInfo = useMemo(() => {
    // 1. Check completed payment transactions
    const paymentTxs = memberTransactions.filter(t => 
      t.status === 'completed' && 
      ['deposit', 'loan_installment', 'share_purchase', 'dps_deposit', 'fdr_deposit', 'fee', 'admission_fee'].includes(t.type)
    );

    // 2. Also check if any loan installment has a paidDate
    let latestLoanInstDate: string | null = null;
    let latestLoanInstAmount = 0;
    let latestLoanNo = '';
    let latestLoanInstNo = 0;

    for (const loan of memberLoans) {
      if (loan.installments) {
        for (const inst of loan.installments) {
          if (inst.status === 'paid' && inst.paidDate) {
            if (!latestLoanInstDate || new Date(inst.paidDate).getTime() > new Date(latestLoanInstDate).getTime()) {
              latestLoanInstDate = inst.paidDate;
              latestLoanInstAmount = inst.paidAmount ?? inst.amount;
              latestLoanNo = loan.loanNo;
              latestLoanInstNo = inst.installmentNo;
            }
          }
        }
      }
    ...
    if (paymentTxs.length === 0 && !latestLoanInstDate) {
      return null;
    }

    // Sort payment transactions by date descending
    const sortedTxs = [...paymentTxs].sort((a, b) => {
    ...
    // If loan installment payment date is newer than latestTx.date
    if (latestLoanInstDate && (!latestTx || new Date(latestLoanInstDate).getTime() > new Date(latestTx.date).getTime())) {
      return {
        date: latestLoanInstDate,
        amount: latestLoanInstAmount,
        type: 'loan_installment' as const,
        typeLabelBn: `ঋণের কিস্তি #${toBengaliNumber(latestLoanInstNo)}`,
        typeLabelEn: `Loan Installment #${latestLoanInstNo}`,
        voucherNo: latestLoanNo ? `LOAN-${latestLoanNo}` : undefined,
      };
    }

    if (latestTx) {
      const typeInfoBn = getTransactionTypeName(latestTx.type, true);
      const typeInfoEn = getTransactionTypeName(latestTx.type, false);
      return {
        date: latestTx.date,
        amount: Number(latestTx.amount) || 0,
        type: latestTx.type,
        typeLabelBn: typeInfoBn.label,
        typeLabelEn: typeInfoEn.label,
        voucherNo: latestTx.voucherNo,
        method: latestTx.paymentMethod,
        txId: latestTx.id,
      };
    }

    return null;
  }, [memberTransactions, memberLoans, isBn]);
  ...
  const getRelativeTime = (dateStr: string) => {
    ...
    try {
      ...
      if (diffDays === 0) return isBn ? 'আজকে' : 'Today';
      if (diffDays === 1) return isBn ? 'গতকাল' : 'Yesterday';
      if (diffDays > 1 && diffDays < 30) {
        return isBn ? `${toBengaliNumber(diffDays)} দিন আগে` : `${diffDays} days ago`;
      }
      if (diffDays >= 30 && diffDays < 365) {
        const months = Math.floor(diffDays / 30);
        return isBn ? `${toBengaliNumber(months)} মাস আগে` : `${months} mo ago`;
      }
      if (diffDays >= 365) {
        const years = Math.floor(diffDays / 365);
        return isBn ? `${toBengaliNumber(years)} বছর আগে` : `${years} yr ago`;
      }
      return '';
    } catch (_) {
      return '';
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
        ...
        {/* Visual Summary Card: Instant Overview for Admins (Total Savings, Active Loan Amount, Last Payment Date) */}
        {hasFinancialAccess && (
          <div 
            id="admin-member-financial-summary"
            className="p-4 sm:p-5 bg-gradient-to-b from-slate-50/95 via-white to-slate-50/50 dark:from-slate-900/90 dark:via-slate-900 dark:to-slate-900/60 border-b border-slate-200 dark:border-slate-800 transition-colors"
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-3.5 pb-2.5 border-b border-slate-200/80 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-blue-100 dark:bg-blue-950/70 text-blue-700 dark:text-blue-300 rounded-lg shrink-0">
                  <ShieldCheck className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                    <span>{isBn ? 'সদস্যের আর্থিক সারসংক্ষেপ (Admin Financial Overview)' : 'Member Financial Overview'}</span>
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60">
                      ...
                      <span>{isBn ? 'রিয়েল-টাইম হিসাব' : 'Live Real-Time'}</span>
                    </span>
                  </h3>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    {isBn 
                      ? 'মোট সঞ্চয় স্থিতি, চলতি ঋণের বকেয়া এবং সর্বশেষ পরিশোধের সার্বিক বিবরণ'
                      : 'Instant overview of total savings balance, active loan amount, and last payment date'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
                <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 font-mono">
                  {isBn ? `সদস্য আইডি: #${member.memberNo}` : `Member ID: #${member.memberNo}`}
                </span>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  ...
                  {member.status === 'active' ? (isBn ? 'সক্রিয় সদস্য' : 'Active') : (isBn ? 'নিষ্ক্রিয়' : 'Inactive')}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="bg-white dark:bg-slate-800/90 rounded-xl p-4 sm:p-5 border border-slate-200 dark:border-slate-700/80 shadow-2xs hover:shadow-xs transition-all flex flex-col justify-between group">
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 uppercase tracking-wider">
                      ...
                      <span>{isBn ? 'মোট সঞ্চয় স্থিতি' : 'Total Savings Balance'}</span>
                    </span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60">
                      {isBn ? 'সক্রিয় সঞ্চয়' : 'Active Savings'}
                    </span>
                  </div>
                  ...
                  <div className="mt-1">
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1.5 flex-wrap">
                      <span>{isBn ? 'আসল জমা:' : 'Principal:'}</span>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-2">
                  <span className="text-[11px] text-slate-400 dark:text-slate-400">
                    {member.shareCount ? (isBn ? `${toBengaliNumber(member.shareCount)}টি শেয়ারযুক্ত` : `${member.shareCount} Shares`) : (isBn ? 'সাধারণ সদস্য' : 'General Member')}
                  </span>
                  {isUserAdmin && (
                    <button
                      type="button"
                      onClick={() => setShowDepositModal(true)}
                      className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition-all shadow-2xs cursor-pointer active:scale-95"
                    >
                      <ArrowDownRight className="w-3.5 h-3.5" />
                      <span>{isBn ? 'টাকা জমা' : 'Deposit'}</span>
                    </button>
              ...
              {/* Card 2: Active Loan Amount */}
              <div className="bg-white dark:bg-slate-800/90 rounded-xl p-4 sm:p-5 border border-slate-200 dark:border-slate-700/80 shadow-2xs hover:shadow-xs transition-all flex flex-col justify-between group">
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 uppercase tracking-wider">
                      <CreditCard className="w-4 h-4 text-rose-600 dark:text-rose-400" />
                      <span>{isBn ? 'চলতি ঋণের বকেয়া' : 'Active Loan Amount'}</span>
                    </span>
                    {member.activeLoanBalance > 0 ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/60">
                        {isBn ? 'বকেয়া চলমান' : 'Active Due'}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60">
                        {isBn ? 'পরিশোধিত / কোনো ঋণ নেই' : 'No Active Due'}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-1">
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                      {member.activeLoanBalance > 0 && primaryActiveLoan ? (
                        <span className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-semibold text-slate-700 dark:text-slate-200">
                            {isBn ? `ঋণ নং: #${primaryActiveLoan.loanNo}` : `Loan #${primaryActiveLoan.loanNo}`}
                          </span>
                          <span className="text-slate-300 dark:text-slate-600">•</span>
                          <span>
                            {isBn ? `${toBengaliNumber(activeLoanRemainingInstallments)}টি কিস্তি বাকি` : `${activeLoanRemainingInstallments} installments left`}
                          </span>
                        </span>
                      ) : (
                        <span>{isBn ? 'সদস্যের সকল ঋণ পরিশোধিত অথবা নতুন ঋণ আবেদনযোগ্য' : 'Member has no outstanding balance or is eligible for a new loan'}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-2">
                  <span className="text-[11px] text-slate-400 dark:text-slate-400">
                    {memberLoans.length > 0 
                      ? (isBn ? `মোট ঋণ গ্রহণ: ${toBengaliNumber(memberLoans.length)}টি` : `Total Loans: ${memberLoans.length}`) 
                      : (isBn ? 'কোনো ঋণ ইতিহাস নেই' : 'No loan history')}
                  </span>
                  {isUserAdmin && (
                    member.activeLoanBalance > 0 ? (
                      <button
                        type="button"
                        onClick={() => {
                          if (setShowQuickKistiModal) {
                            setShowQuickKistiModal(true);
                          } else {
                            setActiveTab('loans');
                          }
                        }}
                        className="inline-flex items-center gap-1 px-2.5 py-1 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold transition-all shadow-2xs cursor-pointer active:scale-95"
                      >
                        <CreditCard className="w-3.5 h-3.5" />
                        <span>{isBn ? 'কিস্তি জমা' : 'Pay Kisti'}</span>
                      </button>
                    ) : (
                      <button
                        type="button"
                        onClick={() => setShowLoanModal(true)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition-all shadow-2xs cursor-pointer active:scale-95"
                      >
              ...
              {/* Card 3: Last Payment Date */}
              <div className="bg-white dark:bg-slate-800/90 rounded-xl p-4 sm:p-5 border border-slate-200 dark:border-slate-700/80 shadow-2xs hover:shadow-xs transition-all flex flex-col justify-between group">
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 uppercase tracking-wider">
                      <Clock className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                      <span>{isBn ? 'সর্বশেষ পরিশোধের তারিখ' : 'Last Payment Date'}</span>
                    </span>
                    {lastPaymentInfo ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60">
                        {getRelativeTime(lastPaymentInfo.date) || (isBn ? 'পরিশোধিত' : 'Recorded')}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                        {isBn ? 'পেমেন্ট নেই' : 'No Payments'}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-1">
                    <div className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                      {lastPaymentInfo ? (
                        formatBengaliDate(lastPaymentInfo.date, isBn)
                      ) : (
                        <span className="text-slate-400 dark:text-slate-500 text-base sm:text-lg font-bold">
                          {isBn ? 'কোনো লেনদেন রেকর্ড নেই' : 'No payment recorded'}
                        </span>
                      )}
                    </div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">
                      {lastPaymentInfo ? (
                        <span className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-emerald-600 dark:text-emerald-400">
                            ৳{formatCurrency(lastPaymentInfo.amount, isBn && useBengaliDigits)}
                          </span>
                          <span className="text-slate-300 dark:text-slate-600">•</span>
                          <span className="font-medium text-slate-700 dark:text-slate-200">
                            {isBn ? lastPaymentInfo.typeLabelBn : lastPaymentInfo.typeLabelEn}
                          </span>
                          {lastPaymentInfo.voucherNo && (
                            <>
                              <span className="text-slate-300 dark:text-slate-600">•</span>
                        ...
                        <span>{isBn ? 'সদস্য এখনও কোনো সঞ্চয় বা কিস্তি প্রদান করেননি' : 'Member has not made any deposit or loan payment yet'}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between gap-2">
                  <span className="text-[11px] text-slate-400 dark:text-slate-400">
                    {isBn ? `মোট লেনদেন: ${toBengaliNumber(memberTransactions.length)}টি` : `Total Transactions: ${memberTransactions.length}`}
                  </span>
                  <button
                    type="button"
                    onClick={() => setActiveTab('passbook')}
                    className="inline-flex items-center gap-1 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-bold text-xs transition-colors cursor-pointer group-hover:underline"
                  >
                    <span>{isBn ? 'পাসবুক ও লেজার' : 'View Passbook'}</span>
