...
import {
  ...
  calculateActiveLoansOut,
  calculateTotalBusinessCapital,
  calculateMemberSavingsBreakdown
...
const sanitizeBusinessProfitRecord = (p: any): BusinessProfitRecord => {
  return {
    ...
    profitAmount: p?.profitAmount !== undefined ? Number(p?.profitAmount) : Number(p?.somitiProfitAmount || 0),
    date: p?.date || new Date().toISOString().split('T')[0],
    totalDepositSnapshot: p?.totalDepositSnapshot !== undefined ? Number(p?.totalDepositSnapshot) : undefined,
    profitRatio: p?.profitRatio !== undefined ? Number(p?.profitRatio) : undefined,
    memberDistributions: Array.isArray(p?.memberDistributions) ? p.memberDistributions : undefined,
    totalDistributed: p?.totalDistributed !== undefined ? Number(p?.totalDistributed) : undefined,
    status: p?.status || 'completed',
    recordedByRole: p?.recordedByRole || 'admin',
...
interface SomitiContextType {
  ...
  approveBusinessProfitRecord: (recordId: string, distributeNow?: boolean) => Promise<{ success: boolean; message: string; record?: BusinessProfitRecord }>;
  rejectBusinessProfitRecord: (recordId: string, reason?: string) => Promise<{ success: boolean; message: string }>;
...
export const SomitiProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    reconciledMembers.forEach(rm => {
      ...
      if (raw) {
        const needsSync = 
          ...
          raw.dpsSavingsBalance !== rm.dpsSavingsBalance ||
          raw.fdrSavingsBalance !== rm.fdrSavingsBalance ||
        ...
        if (needsSync) {
          safeSetDoc(doc(db, 'members', rm.id), {
            ...
            dpsSavingsBalance: rm.dpsSavingsBalance,
            fdrSavingsBalance: rm.fdrSavingsBalance,
  ...
  const closeMemberShares = async (params: {
    ...
    // Update state
    const nextClosures = [closureRecord, ...shareClosures];
    const nextTxs = [newTx, ...transactions];

    setShareClosures(nextClosures);
    setTransactions(nextTxs);

    const recalculated = recalculateMemberShareFinancials(member, nextTxs, nextClosures, unitPrice);
    ...
    try {
      ...
      await safeSetDoc(doc(db, 'transactions', newTx.id), newTx);
      await safeSetDoc(doc(db, 'members', member.id), recalculated);
  ...
  const buyMemberShares = async (params: {
    ...
    // Share Capital is NOT required for Share Purchase; buying shares does NOT automatically create or increase Share Capital
    const newShareValue = member.shareValue || 0;
    const prevSavings = Number(member.totalSavings) || ((member.generalSavingsBalance || 0) + (member.dpsSavingsBalance || 0) + (member.fdrSavingsBalance || 0) + newShareValue);
    const newTotalSavings = prevSavings + totalAmount;
    ...
    // Auto-notes summary of each share's purchase deposit
    ...
    const autoNotes = params.notes?.trim()
      ...
      : isExistingShareDeposit
        ? `শেয়ার #${selectedShares.join(', ')} এর জমা [${breakdownStr}]${isMemberRole ? ' [অনুমোদন অপেক্ষমাণ]' : ''}`
    ...
    const recalculated = recalculateMemberShareFinancials(member, nextTxs, shareClosures, unitPrice);
  ...
  const addDeposit = (params: {
    ...
    // Update Member balance using authoritative recalculation
    setMembers(prev => prev.map(m => {
      if (m.id !== params.memberId) return m;
      const recalculated = recalculateMemberShareFinancials(
        m,
        nextTxs,
        shareClosures,
        settings?.sharePricePerUnit || 1000
      );
      safeSetDoc(doc(db, 'members', m.id), recalculated).catch(console.error);
      safeSetDoc(doc(db, 'memberFinancials', m.id), {
        memberId: m.id,
        memberNo: m.memberNo,
        generalSavingsBalance: recalculated.generalSavingsBalance,
  ...
  const addWithdrawal = (params: {
    ...
    // Deduct from member balance using authoritative recalculation
    setMembers(prev => prev.map(m => {
      if (m.id !== params.memberId) return m;
      const recalculated = recalculateMemberShareFinancials(
        m,
  ...
  const updateTransaction = (id: string, updates: Partial<Transaction>) => {
    ...
    // 1. Member balances adjustment using authoritative recalculation
    if (existingTx.memberId) {
      setMembers(prev => prev.map(m => {
        if (m.id !== existingTx.memberId) return m;

        let loanBal = m.activeLoanBalance;
        if (existingTx.type === 'loan_installment') {
          loanBal = Math.max(0, loanBal - amountDiff);
        }

        const recalculated = recalculateMemberShareFinancials(
          { ...m, activeLoanBalance: loanBal },
          nextTxs,
          shareClosures,
          unitPrice
        );
        ...
        safeSetDoc(doc(db, 'memberFinancials', m.id), {
          memberId: m.id,
          memberNo: m.memberNo,
          generalSavingsBalance: updated.generalSavingsBalance,
          dpsSavingsBalance: updated.dpsSavingsBalance,
          fdrSavingsBalance: updated.fdrSavingsBalance,
          shareCount: updated.shareCount,
          shareValue: updated.shareValue,
  ...
  const deleteTransaction = (id: string) => {
    ...
    let nextClosures = shareClosures;
    if (tx.type === 'share_surrender') {
      nextClosures = shareClosures.filter(c => c.memberId !== tx.memberId || (c.closureDate !== tx.date && c.transactionId !== tx.id));
      setShareClosures(nextClosures);
      shareClosures.filter(c => c.memberId === tx.memberId && (c.closureDate === tx.date || c.transactionId === tx.id)).forEach(c => {
        deleteDoc(doc(db, 'shareClosures', c.id)).catch(console.error);
      });
    }

    // 1. Revert and recalculate member balances strictly by summing valid active financial records
    if (tx.memberId) {
      setMembers(prev => prev.map(m => {
        if (m.id !== tx.memberId) return m;
        let activeLoan = m.activeLoanBalance;
        if (tx.type === 'loan_installment') activeLoan += tx.amount;
        if (tx.type === 'loan_disbursed') activeLoan = Math.max(0, activeLoan - tx.amount);

        const recalculated = recalculateMemberShareFinancials(
          { ...m, activeLoanBalance: activeLoan },
          nextTxs,
          nextClosures,
          unitPrice
        );
  ...
  const approvePendingTransaction = async (txId: string): Promise<{ success: boolean; message: string }> => {
    ...
    // 1. Process balance/shares mutations based on tx.type using authoritative recalculation
    ...
    const nextTxs = transactions.map(t => t.id === tx.id ? updatedTx : t);

    let nextClosures = shareClosures;
    if (tx.type === 'share_surrender') {
      const unitPrice = tx.unitPrice || settings.sharePricePerUnit || 1000;
      const sharesClosed = tx.shareCount || (tx.selectedShares ? tx.selectedShares.length : Math.max(1, Math.round(tx.amount / unitPrice)));
      const existingClosure = shareClosures.find(c => c.transactionId === tx.id || (c.memberId === tx.memberId && c.closureDate === tx.date));
      if (!existingClosure && member) {
        const remainingShares = Math.max(0, (member.shareCount || 0) - sharesClosed);
        const closureRecord: ShareClosure = {
          id: `sc-${Date.now()}`,
          memberId: member.id,
          memberName: member.name,
          memberNo: member.memberNo,
          closedSharesCount: sharesClosed,
          unitPrice,
          principalAmount: sharesClosed * unitPrice,
          profitAmount: Math.max(0, tx.amount - (sharesClosed * unitPrice)),
          totalRefundAmount: tx.amount,
          closureDate: tx.date || getTodayDateStr(),
          voucherNo: tx.voucherNo || `SCV-${Date.now().toString().slice(-6)}`,
          paymentMethod: tx.paymentMethod,
          bankAccountId: tx.bankAccountId,
          handledBy: approverName,
          notes: tx.notes || 'অনুমোদিত শেয়ার সমর্পণ',
          remainingActiveShares: remainingShares,
          createdAt: new Date().toISOString(),
        };
        nextClosures = [closureRecord, ...shareClosures];
        setShareClosures(nextClosures);
        safeSetDoc(doc(db, 'shareClosures', closureRecord.id), closureRecord).catch(console.error);
      }
    }
  ...
  // 2. Permanent Accounting Summary (Single Source of Truth derived dynamically from actual transactions)
  // - Available Balance = Total Money Received - Total Money Paid/Invested
  // - Total Member Savings = Total Member Deposits + Total Member Profit
  // - Active Loans Out = Total Loan Principal - Total Principal Repaid
  // - Total Business Capital = Total Business Funding - Total Business Capital Returned
  ...
  const totalBusinessCapital = accounting.totalBusinessCapital;
  const totalCapital = totalBusinessCapital;
  const totalSavingsInSomiti = accounting.totalMemberSavings;
  const totalAvailableBalance = accounting.availableBalance;
  ...
  const totalProfitInflow = accounting.profitReceived + accounting.otherIncomeReceived;
  const totalExpenses = accounting.otherExpenses;
  const totalWithdrawals = accounting.withdrawalsAndRefunds;
  const totalLoansDisbursed = accounting.loansGiven;
  ...
  // Include today's business profit in todayProfit (only approved/completed profits)
  businessProfitRecords.forEach(r => {
    if (r.date === todayStr && r.status !== 'pending' && r.status !== 'rejected') {
  ...
  const recordBusinessProfit = async (data: {
    ...
    try {
      ...
      // Check if submission is by member (requires admin approval)
      const isUserActuallyAdmin = isUserAdmin || currentUser.role === 'admin' || currentUser.role === 'super_admin';
      const isPending = !isUserActuallyAdmin;
  ...
  const approveBusinessProfitRecord = async (
    recordId: string,
    distributeNow: boolean = true
  ): Promise<{ success: boolean; message: string; record?: BusinessProfitRecord }> => {
    try {
      const record = businessProfitRecords.find(p => p.id === recordId);
      if (!record) {
        throw new Error('ব্যবসায়িক লভ্যাংশ রেকর্ডটি পাওয়া যায়নি।');
      }
      if (record.status === 'completed') {
        return { success: true, message: 'এই রেকর্ডটি ইতিমধ্যে অনুমোদিত ও সম্পন্ন হয়েছে।', record };
      }

      const somitiAmount = Number(record.somitiProfitAmount || record.profitAmount) || 0;
      const totalProfit = Number(record.totalBusinessProfit) || somitiAmount;
      const memberAmount = Number(record.memberProfitAmount) || Math.max(0, totalProfit - somitiAmount);
      const recordDate = record.date || getTodayDateStr();
      const providerName = record.memberName;
      ...
      const memberDistributions: MemberProfitShareItem[] = calcResult.memberShares.map(s => ({
        ...
        rawAllocatedProfit: s.rawAllocated,
        allocatedProfit: s.allocatedProfit,
        creditedToSavings: distributeNow && s.allocatedProfit > 0,
        transactionId: `tx-${record.id}-${s.memberId}`,
      }));

      const updatedRecord: BusinessProfitRecord = {
        ...record,
        status: 'completed',
        approvedBy: currentUser.name,
        approvedAt: new Date().toISOString(),
      ...
      setBusinessProfitRecords(prev => {
        const next = prev.map(p => p.id === recordId ? updatedRecord : p);
        localStorage.setItem('bondhu_business_profit_records', JSON.stringify(next));
        return next;
      });
      await safeSetDoc(doc(db, 'businessProfitRecords', recordId), updatedRecord, { merge: true });

      // 3. Update funding stats if linked
      ...
      // 4. Record income voucher for Somiti ledger
      const incomeVoucher: IncomeExpenseItem = {
        id: `v-${record.id}`,
        voucherNo: `V-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
        type: 'income',
        ...
        paymentMethod: 'cash',
        handledBy: currentUser.name,
        notes: `সদস্য ${providerName} (${providerNo}) হতে প্রাপ্ত অনুমোদিত লভ্যাংশ ৳${somitiAmount} [${record.id}]`,
      };
      setVouchers(prev => [incomeVoucher, ...prev]);
      safeSetDoc(doc(db, 'incomeExpenses', incomeVoucher.id), incomeVoucher).catch(console.error);

      // 5. Distribution to Member Savings
      if (distributeNow && calcResult.totalDistributed > 0) {
        const profitMap = new Map<string, number>();
        const newTransactions: Transaction[] = [];
        let curSerial = getNextLedgerSerial();
        ...
        memberDistributions.forEach(item => {
          if (item.allocatedProfit > 0) {
            profitMap.set(item.memberId, item.allocatedProfit);
            const tx: Transaction = {
              id: item.transactionId || `tx-${record.id}-${item.memberId}`,
              serialNo: curSerial++,
              voucherNo: `PR-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
              memberId: item.memberId,
              memberName: item.memberName,
              ...
              verifiedBy: currentUser.name,
              category: 'business_profit_distribution',
              notes: `${providerName} এর লভ্যাংশ (৳${somitiAmount}) হতে মোট জমার আনুপাতিক বণ্টন [${record.id}]`,
              status: 'completed',
            };
            newTransactions.push(tx);
          }
        ...
        // Create associated MonthlyProfitDistribution record
        const distResult: MonthlyProfitDistribution = {
          id: `pd-${record.id}`,
          distributionNo: `PD-${record.month}-${record.id.slice(-4)}`,
          year: parseInt(record.month.split('-')[0], 10) || new Date().getFullYear(),
          month: parseInt(record.month.split('-')[1], 10) || (new Date().getMonth() + 1),
          monthName: `${record.month} লভ্যাংশ বণ্টন`,
          totalSomitiProfitPool: somitiAmount,
          totalWeightedDeposit: calcResult.totalDeposit,
          totalSavingsPool: calcResult.totalDeposit,
          totalMembersDistributed: memberDistributions.filter(d => d.allocatedProfit > 0).length,
          ...
          status: 'completed',
          memberDistributions,
          notes: `${providerName} এর লভ্যাংশ বণ্টন [${record.id}]`,
          createdAt: new Date().toISOString(),
        };
        setProfitDistributions(prev => [distResult, ...prev.filter(d => d.id !== distResult.id)]);
        safeSetDoc(doc(db, 'profitDistributions', distResult.id), distResult).catch(console.error);
      ...
      return {
        success: true,
        message: 'ব্যবসায়িক লভ্যাংশ সফলভাবে অনুমোদন ও সদস্য সঞ্চয়ে বণ্টন করা হয়েছে।',
        record: updatedRecord,
      };
    } catch (err: any) {
      console.error('Failed to approve business profit record:', err);
      return {
        success: false,
        message: err.message || 'ব্যবসায়িক লভ্যাংশ অনুমোদন করতে ব্যর্থ হয়েছে।',
      };
    }
  };

  const rejectBusinessProfitRecord = async (
    recordId: string,
    reason?: string
  ): Promise<{ success: boolean; message: string }> => {
    try {
      ...
      const updatedRecord: BusinessProfitRecord = {
        ...record,
        status: 'rejected',
        rejectionReason: reason || 'প্রশাসক কর্তৃক বাতিল',
        approvedBy: currentUser.name,
        approvedAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
      ...
      return {
        success: true,
        message: 'ব্যবসায়িক লভ্যাংশ আবেদন বাতিল করা হয়েছে।',
      };
    } catch (err: any) {
      console.error('Failed to reject business profit record:', err);
      return {
        success: false,
        message: err.message || 'ব্যবসায়িক লভ্যাংশ বাতিল করতে ব্যর্থ হয়েছে।',
      };
    }
  };
